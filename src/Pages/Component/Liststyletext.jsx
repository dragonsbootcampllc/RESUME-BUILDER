...
function Liststyletext(){
        const [bgchange , setbgchange] = useState('bg-[#fff]');
        const [textchange , settextchange] = useState('text-[#000]');
        function ChangeColor(e){
            if(e && bgchange === 'bg-[#fff]' && textchange ==='text-[#000]'){
                setbgchange('bg-[#000]');
                settextchange("text-[#fff]");
            }
            else{
                setbgchange('bg-[#fff]') ;
                settextchange("text-[#000]");
            }
        }
    ...
    return(
        <Fragment>
        <div className="flex gap-2 items-center p-2 bg-[#eeeeee8a]">
        {styleTextPart1.map((item,i)=>{
            return(
                
                <div onClick={ChangeColor} key={i} className={`${bgchange} ${textchange} p-2 cursor-pointer rounded-sm`}>
                    {item.url}
                </div>
            )
        })}
